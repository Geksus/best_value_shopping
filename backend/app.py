...
def hello():
    ...
    last_item = Item.query.order_by(Item.added_at.desc()).first()
    if last_item is not None and last_item.added_at is not None and datetime.now().timestamp() - last_item.added_at.timestamp() < 86400:
    ...
    for item in items:
        ...
        if existing is not None:
            print(f'Item {filtered_item["id"]} already exists')
            continue
